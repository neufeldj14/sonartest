...
public class ProjectPersistentCacheProviderTest {
  ...
  public void prepare() {
    ...
    when(server.getVersion()).thenReturn("5.3");
    provider = new ProjectPersistentCacheProvider();
  }

  @Test
  public void test_singleton() {
    assertThat(provider.provide(props, mode, key, server)).isEqualTo(provider.provide(props, mode, key, server));
  }

  @Test
  public void test_cache_dir() {
    assertThat(provider.provide(props, mode, key, server).getDirectory().toFile()).exists().isDirectory();
  }
  ...
  public void test_home() {
    ...
    assertThat(provider.provide(props, mode, key, server).getDirectory()).isEqualTo(expected);
  }
}
