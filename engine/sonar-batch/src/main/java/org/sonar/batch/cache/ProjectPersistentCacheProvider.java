...
public class ProjectPersistentCacheProvider extends ProviderAdapter {
  ...
  public PersistentCache provide(GlobalProperties props, DefaultAnalysisMode mode, ProjectKey key, Server server) {
    if (cache == null) {
      ...
      if (mode.isNotAssociated()) {
        builder.setAreaForLocalProject(serverUrl, server.getVersion());
      } else {
        Preconditions.checkNotNull(projectKey);
        builder.setAreaForProject(serverUrl, server.getVersion(), projectKey);
      }
